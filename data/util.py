    ...
    if float32:
        if bit_depth is None:
            if img.dtype == np.uint8:
                ...
                bit_depth = 1
        img_type = str(type(img)) #new
        if img_type=="<class 'NoneType'>":
            print(img)
            print(str(path))
...
def read_img_seq(path):
    ...
    imgs = imgs[:, :, :, [2, 1, 0]]
    imgs = paddle.to_tensor(np.ascontiguousarray(np.transpose(imgs, (0, 3, 1, 2))),paddle.float32)
    return imgs
...
def cubic(x):
    ...
    return (1.5 * absx3 - 2.5 * absx2 + 1) * (
        (absx <= 1).astype(absx.dtype)) + (-0.5 * absx3 + 2.5 * absx2 - 4 * absx + 2) * ((
            (absx > 1).astype(absx.dtype) * (absx <= 2).astype(absx.dtype)).astype(absx.dtype))
...
def calculate_weights_indices(in_length, out_length, scale, kernel, kernel_width, antialiasing):
    ...
    if not math.isclose(weights_zero_tmp.numpy()[0], 0, rel_tol=1e-6):
        indices = paddle.slice(indices,[1], [1], [P - 2])
        #indices = indices.narrow(1, 1, P - 2)
        weights = paddle.slice(weights,[1],[1],[P-2])
        #weights = weights.narrow(1, 1, P - 2)
    if not math.isclose(weights_zero_tmp.numpy()[-1], 0, rel_tol=1e-6):
...
def imresize(img, scale, antialiasing=True):
    ...
    img_aug[:,sym_len_Hs:sym_len_Hs+in_H,:] = img.clone()
    ...
    img_aug[:,:sym_len_Hs,:] = sym_patch_inv.clone()
    ...
    sym_patch_inv = sym_patch.index_select(inv_idx, 2)
    # out_1_aug = paddle.slice(out_1_aug,[2],[0],[sym_len_Ws]).copy_(sym_patch_inv)
    #out_1_aug.narrow(2, 0, sym_len_Ws).copy_(sym_patch_inv)
    out_1_aug[:,:,:sym_len_Ws] = sym_patch_inv.clone()
    ...
    out_1_aug[:,:,sym_len_Ws+in_W:sym_len_Ws+in_W+sym_len_We] = sym_patch_inv.clone()
...
def imresize_np(img, scale, antialiasing=True):
    ...
    sym_patch = img.astype('float32')[:sym_len_Hs, :, :]
    inv_idx = paddle.arange(sym_patch.shape[0] - 1, -1, -1)
    sym_patch_inv = sym_patch.index_select(inv_idx, 0)
    # img_aug.narrow(0, 0, sym_len_Hs).copy_(sym_patch_inv)
    img_aug[:sym_len_Hs,:,:] = sym_patch_inv.astype("float32")

    sym_patch = img.astype('float32')[-sym_len_He:, :, :]
    inv_idx = paddle.arange(sym_patch.shape[0] - 1, -1, -1)
    sym_patch_inv = sym_patch.index_select(inv_idx, 0)
    ...
    out_1_aug[:,:sym_len_Ws,:] = sym_patch_inv.astype("float32")
